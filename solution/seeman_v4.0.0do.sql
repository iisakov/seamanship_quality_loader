...
--	    	false;
	   	...
	   	baseTypeItem_1 integer default 1; baseTypeItem_2 integer default 2; baseTypeItem integer default 2;	item record;	
...
--			call _print('base_item = (' || loc.constants.base_item::text || ')', loc._row_counter, loc.debug, loc._row_counter);
		...
		for loc.contractor in (select c.*, i.x , i.y from world.contractors c join world.islands i on c.island = i.id where c.item in (baseTypeItem, baseTypeItem_1, baseTypeItem_2))
		loop
		...
		for loc.contract in (select c.id, c.contractor, c.quantity, c.payment_sum, c2.island, c2.item from world.contracts c join world.contractors c2 on c.contractor = c2.id)
		loop
			call _print('contract = ' || loc.contract, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
	
		FOR loc.ship IN (select * from world.ships s join world.cargo c on s.id = c.ship where s.player = think.player_id)
		loop
...
--		sell_price = 10;
		...
		buy_price = loc.item.avg_sell_price * 0.4;
--		buy_price = 5;
