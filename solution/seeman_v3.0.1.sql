...
create unlogged table public.foundation (base_island int2, base_item int2, game_step_num int2);
insert into  public.foundation(base_island, base_item , game_step_num) values (null, null, null);
...
--		buy_price = 5;
	...
	--  Первый попавшийся покупатель с типом товара как у вендора
	   	call _get_customer_by_item(loc.baseTypeItem, loc.sell_price, think.player_id, loc.debug, loc.customer);
	   	 ...
	   	 loc.warehouse_total_quantity = coalesce((select sum(s.quantity) from world."storage" s where s.player = think.player_id), 0);
	 	 call _print('all_storage_quantity = (' || loc.warehouse_total_quantity || ')', loc._row_counter, loc.debug, loc._row_counter);
	
	--	Покупаем
	 	raise notice '%', 50000*(10-loc.constants.game_step_num);
...
--			call _print('tbu = ' || loc.vendor, loc._row_counter, loc.debug, loc._row_counter);
	...
	--	Продаём
		...
		then
			for loc.ship in select * from _get_parked_ships_by_player_id(think.player_id) where _get_parked_ships_by_player_id.ship_num < 6 /*Возможно мы сломаемся на том что у нас нет припаркованых кораблей*/
			loop
				if ((select 1 from world.cargo c where c.ship = loc.ship.id) is null) or ((select s.capacity*0.25 from world.ships s where s.id = loc.ship.id) > (select c2.quantity from world.cargo c2 where c2.ship = loc.ship.id)) /*Проверяет корабль, пустой ли он*/
...
--					call _print('-----------------------------------  '|| loc.ship.island::text || _get_vender_array_storage_by_player_id(think.player_id)::text, loc._row_counter, loc.debug, loc._row_counter);
						...
						if loc.ship.island = any (_get_vender_array_storage_by_player_id(think.player_id)) /*Если мы на острове вендора с не пустым складом*/
						then
