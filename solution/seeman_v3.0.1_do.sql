...
create or replace function _get_parked_ships_by_player_id(player_id integer) returns table (id integer, ship_num integer, player integer, island integer)  as  $$ 
	begin
		return query
		select s.id, (select pms.ship_num::integer from iisakov.my_ships pms where pms.ship_id = s.id) as ship_num, s.player, ps.island from world.ships s
...
-- Работа с таблицами
CREATE schema iisakov;
create unlogged table iisakov.memory (id serial PRIMARY KEY, _time float, _money float);

create unlogged table iisakov.foundation (base_island int2, base_item int2, game_step_num int2);
insert into  iisakov.foundation(base_island, base_item , game_step_num) values (null, null, null);

create unlogged table iisakov.my_ships (ship_id int2, ship_num int2);
...
--	    	false;
	...
	--  Получаем текущие деньги
  	...
  	--  Получаем метрики за прошлый think
		select * into loc.beforeThink from iisakov.memory m order by m.id desc limit 1;
	--	Получаем константы
		select * into loc.constants from iisakov.foundation f;
	
	--  Записываем метрики 
		insert into iisakov.memory(_time, _money) values (loc.currentTime, loc.myMoney);
...
--			loc.constants.base_item = 3;
			UPDATE iisakov.foundation SET base_item = loc.constants.base_item WHERE base_item is null;
			baseTypeItem = loc.constants.base_item;
			insert into iisakov.my_ships select s.id, row_number () over (partition by s.player) from world.ships s where s.player = think.player_id order by s.id;
		else
		...
		for loc.parked_ship in (select s.id, (select pms.ship_num::integer from iisakov.my_ships pms where pms.ship_id = s.id), c.quantity,  ps.island, s.speed, s.capacity from world.parked_ships ps join world.ships s on ps.ship = s.id join world.cargo c on ps.ship = c.ship where s.player = think.player_id)
		loop
			call _print('parked_ship = ' || loc.parked_ship, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
		
		for loc.moving_ship in (select s.id, (select pms.ship_num::integer from iisakov.my_ships pms where pms.ship_id = s.id), c.quantity , ms."start", ms.destination, s.speed, s.capacity, ms.arrives_at from world.moving_ships ms join world.ships s on ms.ship = s.id join world.cargo c on ms.ship = c.ship where s.player = think.player_id)
		loop
			call _print('moving_ship = ' || loc.moving_ship, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
	
		for loc.transferring_ship in (select s.id, (select pms.ship_num::integer from iisakov.my_ships pms where pms.ship_id = s.id), ts.island, ts.finish_time, s.speed, s.capacity from world.transferring_ships ts join world.ships s on ts.ship = s.id where s.player =  think.player_id)
		loop
		...
		FOR loc.ship IN (select * from world.ships s join iisakov.my_ships ms on s.id = ms.ship_id where s.player = think.player_id)
		loop
	...
	--	Получаем бвзовый остров 
		...
		then
			...
			UPDATE iisakov.foundation SET base_island = loc.constants.base_island WHERE base_island is null;
...
--		if loc.constants.base_island is null 
--		then
--			insert into iisakov.foundation(base_island) 
...
--		end if;
	--	Проверяем ход игры
		...
		then
			update iisakov.foundation set game_step_num = ceil(currentTime/10000);
		else
...
--		buy_price = 5;
	...
	--	Покупаем   
		if (loc.vendor.id is not null) and (loc.constants.game_step_num < 10)
		then
