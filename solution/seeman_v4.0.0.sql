...
create or replace procedure _sell_all(contractors_id integer, quantity double precision, player_id integer, item integer, debug bool) as $$
	begin
		select c.id, c.quantity into _sell_all.contractors_id, _sell_all.quantity
		from world."storage" s 
		join world.contractors c on s.island = c.island
		where s.player = _sell_all.player_id
			and c."type" = 'customer'
			and c.item = s.item
			order by s.quantity 
			desc limit 1;	
	
		insert into actions.offers(contractor, quantity) values (_sell_all.contractors_id, _sell_all.quantity);
	end
$$ language plpgsql;
...
create or replace procedure _load(ship_id integer, item integer, player integer, island integer, debug bool) as $$
	...
	begin
		loc.quantity := (select s.quantity from world."storage" s where s.island = _load.island and s.player = _load.player and s.item = _load.item);
...
--	  		and (c.player is null or c.player != _get_customer_by_item.player_id)
	  		and c.contractor not in (select c2.contractor from world.contracts c2 where c2.player = _get_customer_by_item.player_id)
...
create or replace function _get_vender_array_storage_by_player_id(player_id integer, item_id integer) returns integer array as  $$
	declare
	test int array;
	begin
		select array_agg(island) into test from (select * from _get_storage_by_player_id(_get_vender_array_storage_by_player_id.player_id) where c_type = 'v' and item = _get_vender_array_storage_by_player_id.item_id order by quantity desc) as t;
		return test; 
	end
$$ language plpgsql;

create or replace function _get_array_storage_by_player_id(player_id integer, item integer) returns integer array as  $$
	declare
	_result int array;
	begin
		select 
		...
		from (select
			  ...
			  where c2."type" = 'customer' and c.player = _get_array_storage_by_player_id.player_id and c2.item = _get_array_storage_by_player_id.item
...
create unlogged table public.foundation (base_island int2, base_item int2, base_item_1 int2, base_item_2 int2, game_step_num int2);
insert into  public.foundation(base_island, base_item, game_step_num, base_item_1, base_item_2) values (null, null, null, null, null);
...
--	    	false;
	   	...
	   	baseTypeItem_1 integer default 1; baseTypeItem_2 integer default 2; baseTypeItem integer default 2;	item_sell record; item_buy record;	
	   	
	   	contractor record;
	   	vendor record;   v_island integer;
	   	customer record; c_island integer;
	   
	   	contract record; events record;
	   	...
	   	warehouse record; warehouse_total_quantity double precision;
	   	
	   	_row_counter int default 0;
	   
	   	trush int array;
	   	count_row_contractor integer;
	begin
...
--		Получаем базовый итем
		if loc.constants.base_item is null
		
--		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		--	Получаем первый базовый итем
		then
			with ct as (
			...
			select count(*) into count_row_contractor/*vt.item, customer_supply, vendor_demand, customer_count, vendor_count*/ from ct
			join vt on ct.item = vt.item
			where vendor_count/customer_count > 0.5
			and vendor_count/customer_count < 2
			and customer_count > 2
			and vendor_count > 2
			and customer_count < 12
			and vendor_count < 12;
			
			if count_row_contractor = 1
			then
			call _print('Здесь', loc._row_counter, loc.debug, loc._row_counter);
				with ct as (
					select c.item, c."type", sum(c.quantity*c.price_per_unit) as customer_supply, count(*) as customer_count from world.contractors c 
					where c."type" = 'customer'
					group by c.item, c."type"
				...
				select vt.item into loc.constants.base_item/*, customer_supply, vendor_demand, customer_count, vendor_count*/ from ct
				join vt on ct.item = vt.item
				where vendor_count/customer_count > 0.5
				and vendor_count/customer_count < 2
				and customer_count > 2
				...
				and customer_count < 12
				and vendor_count < 12;
			elseif count_row_contractor = 0
			then
				call _print('Здесь2', loc._row_counter, loc.debug, loc._row_counter);
				with ct as (
					select c.item, c."type", sum(c.quantity*c.price_per_unit) as customer_supply, count(*) as customer_count from world.contractors c 
					where c."type" = 'customer'
					group by c.item, c."type"
				...
				select coalesce(vt.item, 3) into loc.constants.base_item/*, customer_supply, vendor_demand, customer_count, vendor_count*/ from ct
				join vt on ct.item = vt.item
				where customer_supply/vendor_demand > 3
				order by customer_supply desc
				limit 1;
				if loc.constants.base_item is null
				then
					loc.constants.base_item = 3;
					call _print('------------------------------------------------------ Вот тут у нас '|| loc.constants.base_item::text, loc._row_counter, loc.debug, loc._row_counter);
				end if;
--				loc.constants.base_item = 3;
				
			else
				call _print('Здесь3', loc._row_counter, loc.debug, loc._row_counter);
				with ct as (
					select c.item, c."type", sum(c.quantity*c.price_per_unit) as customer_supply, count(*) as customer_count from world.contractors c 
					where c."type" = 'customer'
					group by c.item, c."type"
				...
				select coalesce(vt.item, 4) into loc.constants.base_item/*, customer_supply, vendor_demand, customer_count, vendor_count*/ from ct
				join vt on ct.item = vt.item
				where customer_supply/vendor_demand > 3
				and vendor_count/customer_count > 0.5
				and vendor_count/customer_count < 2
				and customer_count > 2
				and vendor_count > 2
				and customer_count < 12
...
--				loc.constants.base_item = 3;
			...
			call _print('------------------------------------------------------ Здесь присвоили '|| loc.constants.base_item::text, loc._row_counter, loc.debug, loc._row_counter);
			
			UPDATE public.foundation SET base_item = loc.constants.base_item WHERE base_item is null;
			UPDATE public.foundation SET base_item_1 = loc.constants.base_item WHERE base_item_1 is null;
--		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		--	Получаем второй базовый итем
			with ct as (
				select c.item, c."type", sum(c.quantity*c.price_per_unit) as customer_supply, count(*) as customer_count from world.contractors c 
				where c."type" = 'customer'
				group by c.item, c."type"
			...
			and customer_count < 12
			and vendor_count < 12
			and vt.item <> loc.constants.base_item;
...
--			loc.constants.base_item = 5;
--			UPDATE public.foundation SET base_item = loc.constants.base_item WHERE base_item is null;
--			UPDATE public.foundation SET base_item_1 = loc.constants.base_item WHERE base_item_1 is null;
			UPDATE public.foundation SET base_item_2 = loc.constants.base_item WHERE base_item_2 is null;
			baseTypeItem = loc.constants.base_item;
			baseTypeItem_1 = loc.constants.base_item_1;
			baseTypeItem_2 = loc.constants.base_item_2;
			...
			call _print('base_item = (' || loc.constants.base_item::text || ')', loc._row_counter, loc.debug, loc._row_counter);
			call _print('base_item_1 = (' || loc.constants.base_item_1::text || ')', loc._row_counter, loc.debug, loc._row_counter);
			call _print('base_item_2 = (' || loc.constants.base_item_2::text || ')', loc._row_counter, loc.debug, loc._row_counter);
			baseTypeItem = loc.constants.base_item;
			baseTypeItem_1 = loc.constants.base_item_1;
			baseTypeItem_2 = loc.constants.base_item_2;
		end if;
...
--	
--		for loc.contractor in (select c.*, i.x , i.y from world.contractors c join world.islands i on c.island = i.id where c.item in (baseTypeItem_1, baseTypeItem_2))
--		loop
...
--		end loop;
		...
		for loc.contract in (select c.id, c.contractor, c.quantity, c.payment_sum, c2.island, c2.item, c.player from world.contracts c join world.contractors c2 on c.contractor = c2.id)
		loop
			call _print('contract = ' || loc.contract, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
		
	
		for loc.events in (select cc.contract, c.contractor, c.contractor, c.quantity, c.payment_sum from events.contract_completed cc join world.contracts c on cc.contract = c.id)
		loop
			call _print('event_contract_completed = ' || loc.events, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
	
		for loc.events in (select cs.contract, c.contractor, c.quantity, c.payment_sum from events.contract_started cs join world.contracts c on cs.contract = c.id where c.player = think.player_id)
		loop
			call _print('event_contract_start = ' || loc.events, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
	
		for loc.events in (select * from events.offer_rejected)
		loop
			call _print('event_contract_offer_rejected = ' || loc.events, loc._row_counter, loc.debug, loc._row_counter);
		end loop;
...
--		end loop;
	...
	--	Получаем бвзовый остров 
		...
		then
			loc.constants.base_island = (_cumpute_base_island((select json_agg(t.*) from (
													 select i.id, i.x, i.y from world.islands i
													 join world.contractors c on i.id = c.island
													 group by i.id 
													 having array_to_string(array_agg(c.item::text), ' ') not like '%' || loc.baseTypeItem::text || '%') as t),
					  		   _cumpute_avr_point((select json_agg(i.*) 
					  		                       from world.islands i 
					  		                       join world.contractors c on i.id = c.island
					  		                       where c.item = loc.baseTypeItem))));
					  		                      
			UPDATE public.foundation SET base_island = loc.constants.base_island WHERE base_island is null;
...
--	????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????	
		loc.baseTypeItem = loc.baseTypeItem_1;
		
		call _get_metric_sell_price_by_item(loc.baseTypeItem, loc.item_sell);
		call _get_metric_buy_price_by_item(loc.baseTypeItem, loc.item_buy);
		loc.sell_price = (loc.item_sell.max_sell_price + 3*loc.item_buy.min_buy_price)*0.25;
--		sell_price = 10;
		call _print('sell_price = (' || loc.sell_price::text || ')', loc._row_counter, loc.debug, loc._row_counter);
		loc.buy_price = loc.sell_price*0.4;
--		buy_price = 5;
	...
	--  Первый попавшийся покупатель с типом товара как у вендора
	   	call _get_customer_by_item(loc.baseTypeItem, loc.sell_price, think.player_id, loc.debug, loc.customer);
	   	 ...
	   	 loc.warehouse_total_quantity = coalesce((select sum(s.quantity) from world."storage" s where s.player = think.player_id and s.item = loc.baseTypeItem), 0);
	 	 call _print('all_storage_quantity = (' || loc.warehouse_total_quantity || ')', loc._row_counter, loc.debug, loc._row_counter);
	
	--	Покупаем
	 	
		if (loc.vendor.id is not null) and (loc.constants.game_step_num < 10) and loc.warehouse_total_quantity < 25000*(10-loc.constants.game_step_num)
		then
			call _buy(loc.vendor.id, loc.vendor.quantity, loc.debug);
			call _print('buy = (' || loc.vendor.island::text || ',' || loc.vendor.quantity || ',' || loc.vendor.price_per_unit || ')', loc._row_counter, loc.debug, loc._row_counter);
--			call _print('tbu = ' || loc.vendor, loc._row_counter, loc.debug, loc._row_counter);
	...
	--	Продаём
	   	...
	   	then
	   		...
	   		call _print('sell = (' || loc.customer.island::text || ',' || loc.customer.quantity || ',' || loc.customer.price_per_unit ||')', loc._row_counter, loc.debug, loc._row_counter);
	   	end if;
   		if loc.constants.game_step_num > 9
   		then
	   		call _sell_all(loc.customer.id, loc.customer.quantity, think.player_id, loc.baseTypeItem, loc.debug);
	   		call _print('sell = (' || loc.customer.island::text || ',' || loc.customer.quantity || ',' || loc.customer.price_per_unit ||')', loc._row_counter, loc.debug, loc._row_counter);
	   	end if;
	   
--	????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????   
		
	   	loc.baseTypeItem = loc.baseTypeItem_2;
...
--		buy_price = 5;
	...
	--  Второй попавшийся вендор
	   call _get_vendor_by_item(loc.baseTypeItem, loc.buy_price, loc.debug, loc.vendor);
	
	--  Второй попавшийся покупатель с типом товара как у вендора
	   	call _get_customer_by_item(loc.baseTypeItem, loc.sell_price, think.player_id, loc.debug, loc.customer);
...
--	????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????	   
		...
		then
			...
			loop
				if loc.ship.ship_num > 5
				then
					loc.baseTypeItem = loc.baseTypeItem_1;
				else
					loc.baseTypeItem = loc.baseTypeItem_2;
				end if;
				if ((select 1 from world.cargo c where c.ship = loc.ship.id) is null) or ((select s.capacity*0.5 from world.ships s where s.id = loc.ship.id) > (select c2.quantity from world.cargo c2 where c2.ship = loc.ship.id)) /*Проверяет корабль, пустой ли он*/
				then /* Работаем с путым или теми в ком меньше 0.1 товара */
					if loc.ship.ship_num <= 0  /*Выбираем продающие корабли*/
					then
...
--					call _print('-----------------------------------  '|| loc.ship.island::text || _get_vender_array_storage_by_player_id(think.player_id)::text, loc._row_counter, loc.debug, loc._row_counter);
						...
						if loc.ship.island = any (_get_vender_array_storage_by_player_id(think.player_id, loc.baseTypeItem)) /*Если мы на острове вендора с не пустым складом*/
						then
...
--							call _print('-----------------------------------  '|| array_length(_get_vender_array_storage_by_player_id(think.player_id), 1)::text, loc._row_counter, loc.debug, loc._row_counter);
							if array_length(_get_vender_array_storage_by_player_id(think.player_id, loc.baseTypeItem), 1) > 0 /* есть ли хоть один склад vendora*/
							then
								v_island := (select * from _get_vender_array_storage_by_player_id(think.player_id, loc.baseTypeItem))[1]; /* выбираем самый накаченый */
					...
					then
						...
						then
							call _unload(loc.ship.id, loc.baseTypeItem, loc.debug); /*Разгружаемся на покупателя*/
							call _print('unload = (' || loc.ship.island::text || ')', loc._row_counter, loc.debug, loc._row_counter);
						else
							if array_length(_get_array_storage_by_player_id(think.player_id, loc.baseTypeItem), 1) > 0 /* покупатели существуют */
							then /* Мы НЕ на острове покупателя */
								c_island := (_get_array_storage_by_player_id(think.player_id, loc.baseTypeItem))[1]; /* Берём лучшего покупателя */
